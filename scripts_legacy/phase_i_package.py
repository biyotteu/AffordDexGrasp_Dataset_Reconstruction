...
def generate_qc_report(cfg):
    ...
    for sf in sample_files[:5000]:  # 샘플링
        ...
        contact_parts = sem.get('contact_parts', 'unknown')
        if isinstance(contact_parts, list):
            contact_parts = ', '.join(contact_parts)
        contact_counter[str(contact_parts)] += 1
        direction = sem.get('grasp_direction', 'unknown')
