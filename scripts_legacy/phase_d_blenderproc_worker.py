#!/usr/bin/env python3
"""
BlenderProc scene generation script.
Run with: blenderproc run phase_d_blenderproc_worker.py --job <job.json>
"""
import sys
import json
import argparse
...
# BlenderProc must be imported after blenderproc run
...
def create_tabletop_scene(job, output_dir):
    ...
    # --- 5대 카메라 설정 ---
    cam_cfg = job["camera"]
    intrinsics = bproc.camera.set_intrinsics_from_blender_params(
        lens=35,
