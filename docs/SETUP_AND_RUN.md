...
**참고**: Phase D(Scene 구성)와 Phase E(Paint3D 텍스처)는 서로 독립적이며 병렬 실행 가능합니다. Phase F(렌더링)가 두 Phase의 결과를 합쳐 사용하므로, F 시작 전에 D와 E가 모두 완료되어야 합니다.
...
**Blender 직접 실행**: `blenderproc run` CLI 대신 `blender --background --python worker.py -- --args` 방식으로 실행합니다. conda 환경의 numpy가 Blender 내장 Python에 주입되는 것을 방지하여 버전 충돌을 해결합니다.
...
**텍스처 연결**: Paint3D가 `albedo.png`를 생성하면, worker가 자동으로 `paint3d.mtl`(material 파일)을 생성하고 `textured_mesh.obj`에 `mtllib` 지시문을 삽입합니다. 이 OBJ-MTL 체인을 통해 Phase F의 BlenderProc이 텍스처를 자동으로 로드합니다.

**성공 마커**: `.paint3d_done` 파일로 Paint3D 성공 여부를 구별합니다. `material_*.png` 존재만으로는 판별하지 않습니다.
...
**Blender 직접 실행**: Phase D와 동일하게 `blender --background --python` 방식으로 실행합니다.

**3단계 텍스처 Fallback**: (1) MTL 자동 로드 → (2) Blender 노드에 직접 이미지 텍스처 할당 → (3) 랜덤 단색. 텍스처 소스 정보는 `render_info.json`에 기록됩니다.
